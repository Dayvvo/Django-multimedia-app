...
def index2(request, slug):
    int_value = int(slug) * 2
    first_value = (int(slug)-1) * 2
    articles = Article.objects.all()
    trending = Article.objects.order_by('views').reverse()[:6]
    count = math.floor(articles.count() / 2)
...
def index(request):
    ...
    articles_shave = articles[:3]
    count = math.floor(articles.count() / 3)
    trending = Article.objects.order_by('views').reverse()[:6]
...
def category2(request, slug, slug2):
    int_value = int(slug2) * 2
    first_value = (int(slug2)-1) * 2
    ...
    if int_value < count:
        int_value -= count
        article_query = Article.objects.filter(genre=slug).order_by('date')[:int_value][::-1]
    else:
        article_query = Article.objects.filter(genre=slug).order_by('date').reverse()[first_value:int_value]
    next_location = request.POST.get('next')
    form = Contactform(request.POST or None)
    if form.is_valid():
        form.save()
...
def by_category(request, slug):
    next_location = request.POST.get('next')
    article_set = Article.objects.filter(genre=slug).order_by('date').reverse()[:2]
...
def article_detail(request, slug):
    ...
    latest = Article.objects.order_by('date').reverse()
    trending = Article.objects.order_by('views').reverse()[:6]
